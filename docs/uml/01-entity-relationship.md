...
- **NodeVersion** fields are `id`, `nodeId`, `versionNumber`, `nodeDetailsSnapshot`, and `createdBy`.
- **AttributeKey enum** values are `contains`, `depends_on`, `references`, `parent_of`, `relates_to`.
