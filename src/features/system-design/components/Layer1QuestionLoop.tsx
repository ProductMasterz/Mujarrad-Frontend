...
export function Layer1QuestionLoop() {
  ...
  const { questions, understanding, completeness, task4AiUsage } = graphState;
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/70">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">Task 4</p>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Review the current understanding and completeness status while the clarification
            conversation continues in the assistant panel.
          </p>
        </div>
      ...
      <Task4AiUsagePanel calls={task4AiUsage.calls} />

      <QuestionHistory questions={questions} conversation={graphState.conversation} />
    </div>
  );
}
