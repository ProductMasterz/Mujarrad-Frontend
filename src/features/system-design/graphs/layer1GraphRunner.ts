import type {
  Layer1GraphEvent,
  Layer1GraphResult,
  Layer1GraphState,
} from '../types/graph.types';
import type { QuestionAnswer } from '../types/layer1.types';
import { processSystemDesignInput } from '../tools/inputProcessingTool';
import { createIsoTimestamp, createSystemDesignId } from '../utils/id';
import {
  completeLayer1Step,
  createInitialLayer1GraphState,
} from './layer1GraphState';
import { generateQuestionNode } from '../nodes/generateQuestionNode';
import { updateUnderstandingNode } from '../nodes/updateUnderstandingNode';
import { checkCompletenessNode } from '../nodes/checkCompletenessNode';
import { isReadyForDiagram } from '../utils/completeness';

function addGraphError(
  state: Layer1GraphState,
  message: string,
  source: string,
): Layer1GraphState {
  return {
    ...state,
    errors: [
      ...state.errors,
      {
        id: createSystemDesignId('layer1-error'),
        message,
        source,
        createdAt: createIsoTimestamp(),
      },
    ],
    nextAction: 'error',
    updatedAt: createIsoTimestamp(),
  };
}

export async function runLayer1GraphEvent(
  event: Layer1GraphEvent,
  existingState?: Layer1GraphState,
): Promise<Layer1GraphResult> {
  let state = existingState ?? createInitialLayer1GraphState();

  if (event.type === 'start_run') {
    return { ok: true, state, message: 'Layer 1 run initialized.' };
  }

  if (event.type === 'reset_run') {
    return { ok: true, state: createInitialLayer1GraphState(), message: 'Layer 1 run reset.' };
  }

  if (event.type === 'submit_input') {
    if (!event.rawInput) {
      return { ok: false, state: addGraphError(state, 'Missing raw input.', 'layer1GraphRunner'), message: 'Missing raw input.' };
    }

    const processingResult = processSystemDesignInput(event.rawInput);

    if (!processingResult.processedInput) {
      return {
        ok: false,
        state: {
          ...addGraphError(state, processingResult.errors[0] ?? 'Input processing failed.', 'process_input'),
          rawInputs: [...state.rawInputs, event.rawInput],
        },
        processingResult,
        message: processingResult.errors[0] ?? 'Input processing failed.',
      };
    }

    const completedState = completeLayer1Step(
      {
        ...state,
        rawInputs: [...state.rawInputs, event.rawInput],
        processedInput: processingResult.processedInput,
        stage: 'clarification',
        nextAction: 'ask_question',
        updatedAt: createIsoTimestamp(),
      },
      'input',
    );

    return {
      ok: true,
      state: { ...completedState, nextAction: 'ask_question' },
      processingResult,
      message: 'Input processed. Clarification is available.',
    };
  }

  if (event.type === 'generate_question') {
    const { question, error } = await generateQuestionNode(state);
    
    if (error || !question) {
      return {
        ok: false,
        state: addGraphError(state, error || 'Failed to generate question', 'generate_question'),
        message: error || 'Failed to generate question',
      };
    }

    return {
      ok: true,
      state: {
        ...state,
        currentQuestion: question,
        questions: [...state.questions, question],
        nextAction: 'wait_for_answer',
        updatedAt: createIsoTimestamp(),
      },
      message: 'Generated next clarification question.',
    };
  }

  if (event.type === 'submit_answer') {
    if (!event.answer || !state.currentQuestion) {
      return {
        ok: false,
        state: addGraphError(state, 'Missing answer or current question.', 'submit_answer'),
        message: 'Missing answer or current question.',
      };
    }

    // 1. Save answer
    const newAnswer: QuestionAnswer = {
      id: createSystemDesignId('answer'),
      questionId: state.currentQuestion.id,
      answer: event.answer,
      createdAt: createIsoTimestamp(),
    };

    state = {
      ...state,
      qaHistory: [...state.qaHistory, newAnswer],
      currentQuestion: {
        ...state.currentQuestion,
        answeredAt: createIsoTimestamp(),
        answer: event.answer,
      },
    };

    // 2. Update Understanding
    const { understanding, error: updateErr } = await updateUnderstandingNode(state);
    if (updateErr) {
      return { ok: false, state: addGraphError(state, updateErr, 'update_understanding'), message: updateErr };
    }
    state = { ...state, understanding };

    // 3. Check Completeness
    const { completeness, error: compErr } = await checkCompletenessNode(state);
    if (compErr) {
      return { ok: false, state: addGraphError(state, compErr, 'check_completeness'), message: compErr };
    }
    state = { ...state, completeness };

    // 4. Decide next action
    if (isReadyForDiagram(completeness)) {
      const completedState = completeLayer1Step(state, 'clarification');
      return {
        ok: true,
        state: { ...completedState, nextAction: 'generate_spec' },
        message: 'Clarification complete. Ready for specification.',
      };
    } else {
      return {
        ok: true,
        state: { ...state, nextAction: 'ask_question' },
        message: 'More clarification needed.',
      };
    }
  }

  if (event.type === 'complete_step') {
    if (!event.stepId) {
      return { ok: false, state: addGraphError(state, 'Missing step id.', 'complete_step'), message: 'Missing step id.' };
    }
    return { ok: true, state: completeLayer1Step(state, event.stepId), message: `${event.stepId} completed.` };
  }

  return { ok: true, state, message: 'State synchronized.' };
}
